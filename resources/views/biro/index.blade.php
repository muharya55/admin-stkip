    ...
    <div class="row">
       <div class="col-md-12">
          <div class="card border-0 shadow rounded">
             <div class="card-body"> 
                <a href="/biro/create" class="btn btn-md btn-success mb-3">TAMBAH DATA</a>
                {{-- <a href="{{ route('biro.import') }}" class="btn btn-md btn-primary mb-3">IMPORT DATA SISWA</a> --}}
                        ...
                        <div class="table-responsive">
                           <table class="table table-bordered  no-footer" id="DataTables_Table_0" aria-describedby="DataTables_Table_0_info">
                              <thead>
                                <tr>
                                      <th>No</th>
                                      <th>Gambar Ketua Biro</th>
                                      <th>Nama Biro</th>
                                      <th>Nama Ketua Biro</th>
                                      <th>Email Ketua Biro</th> 
                                      <th>Aksi</th>
                                </tr>
                             </thead>
                              <tbody>
                                ...
                                @foreach($datas as $key => $item)
                                 <tr class="odd">
                                    <td class="sorting_1">{{ $no++ }}</td>
                                      <td>
                                       @if ($item->gambar_pimpinan)
                                             <img src="{{ asset('storage/' . $item->gambar_pimpinan) }}" width="80">
                                       @else
                                             <em>Tidak ada</em>
                                       @endif
                                    </td>
                                    <td class="text-wrap" style="max-width: 700px;">{{ $item->nama }} </td>
                                    <td class="text-wrap" style="max-width: 700px;">{{ $item->nama_pimpinan }} </td>
                                    <td class="text-wrap" style="max-width: 700px;">{{ $item->email_pimpinan }} </td>
                                   
                                   <td class="text-center d-flex">
                                      <a href="{{ route('biro.edit', $item->id) }}" data-bs-toggle="tooltip" title="Edit" class="p-2"><i class="fa fa-edit text-primary "></i></a>
                                      ...
                                      <form action="{{ route('biro.destroy', $item->id) }}" id="delete{{ $item->id }}" method="POST">
                                            @csrf
                                            @method('delete')
                                      </form>
