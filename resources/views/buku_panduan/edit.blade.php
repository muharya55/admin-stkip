         ...
         <div class="row">
            ...
            <div class="col-md-12">
               <div class="card border-0 shadow rounded">
                  <div class="card-body">
                    <form action="{{ route('buku-panduan.update', $data->id) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('PATCH')
                        @component('components.textInput',['label'=>'Nama ' ,'name'=>'nama','value'=>$data->nama ]) @endcomponent
                        @component('components.textInput',['label'=>'Tahun' ,'name'=>'tahun','value'=>$data->tahun  ]) @endcomponent
                        <div class="mb-3">
                            <label class="form-label">File Lama</label><br>
                            <a href="{{ asset('storage/buku_panduan/' . $data->nama_file) }}" target="_blank">{{ $data->nama_file }}</a>
                        </div>
